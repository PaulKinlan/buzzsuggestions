...
_TWITTER_OAUTH_BASE = "http://twitter.com/oauth"
...
class Twitter(object):
    ...
    def verify(self, id, **args):
        ...
        return self.fetch("/verify_credentials.json", post_args=args)

    @_authenticated
    def follow(self, feed, **args):
    ...
    def fetch(self, path, post_args=None, **args):
        ...
        if args: url += "?" + urllib.urlencode(args, True)
        if post_args is not None:
            request = urllib2.Request(url, urllib.urlencode(post_args, True))
        else:
...
def get_oauth_request_token_url(consumer_token):
    ...
    """
    url = _TWITTER_OAUTH_BASE + "/request_token"
    args = dict(
...
def get_oauth_authorization_url(request_token):
    ...
    """
    return _TWITTER_OAUTH_BASE + "/authorize?" + \
...
def get_oauth_authentication_url(request_token):
    ...
    See http://oauth.net/core/1.0/#auth_step2
    """
    return _TWITTER_OAUTH_BASE + "/authenticate?" + \
        urllib.urlencode(dict(oauth_token=request_token["key"]))
...
def get_oauth_access_token_url(consumer_token, request_token):
    ...
    """
    url = _TWITTER_OAUTH_BASE + "/access_token"
    args = dict(
        oauth_consumer_key=consumer_token["key"],
